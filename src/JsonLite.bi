...
'#                                                       #
'#  Minimal, dependency-free JSON parser/serializer for  #
'#  the Agent MCP command channel. Shared by ilwaco (the #
'#  AgentPipe server) and the ilwaco-mcp sidecar, so it  #
'#  must not depend on MFF or any shared library. Strings #
'#  are UTF-8 throughout (the socket and MCP both speak   #
'#  UTF-8); callers convert to/from WString at their own  #
'#  boundary.                                             #
'#                                                       #
'#  DOM shape follows cJSON: each value owns a first-    #
'#  Child pointer and a NextSib pointer (object members  #
'#  carry Name). Delete of the root frees the whole tree #
'#  via the destructor.                                  #
'#########################################################
#pragma once
