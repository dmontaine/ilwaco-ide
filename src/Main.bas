...
Dim Shared As Dictionary Helps, HotKeys, Compilers, MakeTools, Debuggers, Terminals, OtherEditors, BuildConfigurations, AIAgents
...
Function Compile(Parameter As String = "", bAll As Boolean = False) As Integer
	...
	For k As Integer = 0 To NodesCount - 1
		...
		If lvProblems.ListItems.Count <> 0 Then
			tpProblems->Caption = ML("Problems") & IIf(NumberErr + NumberWarning + NumberInfo > 0, " (" & WStr(NumberErr + NumberWarning + NumberInfo) & " " & ML("Pos") & ")", WStr(""))
			Dim As UString Problems
			Problems = IIf(NumberErr > 0, ML("Errors") & " (" & WStr(NumberErr) & " " & ML("Pos") & ")", WStr(""))
			Problems &= IIf(NumberWarning > 0, IIf(Problems = WStr(""), WStr(""), WStr(", ")) & ML("Warnings") & " (" & WStr(NumberWarning) & " " & ML("Pos") & ")", WStr(""))
			Problems &= IIf(NumberInfo > 0, IIf(Problems = WStr(""), WStr(""), WStr(", ")) & ML("Messages") & " (" & WStr(NumberInfo) & " " & ML("Pos") & ")", WStr(""))
...
Sub ChangeUseDebugger(bUseDebugger As Boolean, ChangeObject As Integer = -1)
	Static As Boolean bInChange = False
...
End If
WLet(AISystem_PromoptPtr, "Please use English for your responses unless otherwise instructed." & _
...
Public Sub AIResetContext()
	...
	"{""role"": ""user"", ""content"": """ & "Please confirm the context has been reset." & """}]}"
...
Function GetBundledCompilerFolder() As UString
	Return ExePath & Slash & BUNDLED_COMPILER_FOLDER
End Function

Function GetBundledCompilerExe() As UString
	Return GetBundledCompilerFolder() & Slash & BUNDLED_COMPILER_EXE
End Function

Sub SetBundledCompilerPath()
	Dim As UString CompilerExe = GetFullPath(GetBundledCompilerExe())
	WLet(Compiler32Path, CompilerExe)
	WLet(Compiler64Path, CompilerExe)
End Sub

Function CheckCompilerPaths As Boolean
	Dim As UString CompilerExe = GetFullPath(GetBundledCompilerExe())
	If FileExists(CompilerExe) Then
		SetBundledCompilerPath()
		Return True
	End If
		Dim As ZString Ptr PathFbc = g_find_program_in_path("fbc")
		If PathFbc <> NULL Then
			CompilerExe = WStr(*PathFbc)
			g_free(PathFbc)
		End If
	If FileExists(CompilerExe) Then
		SetBundledCompilerPath()
		Return True
	End If
	ShowMessages ML("Compiler not found") & ": " & CompilerExe
	Return False
End Function
