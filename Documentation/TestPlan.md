...
| T14 | A project created from a template compiles and prints correctly (BOM regression) | **PARTIAL** 2026-08-04 — **GUI Application PASSES**: compiles clean and its window opens with a correctly-rendered caption (so no wide-literal BOM effect). **Console Application FAILS**: `mff/Console.bi` is Win32-only (84 console-API calls, no Linux branch) and its `windows.bi` include drags in `-lkernel32/-lgdi32/-luser32…`, so the link fails. Two MFF bugs found and fixed on the way (`NoInterface.bi`); the Console template itself is unresolved — see PROJECT_STATUS |
| T15 | Every project type offered by New Project compiles | **PARTIAL** 2026-08-04 — GUI, GTK, Dynamic Library, Static Library and Control Library compile; Console Application does not (row above) |
