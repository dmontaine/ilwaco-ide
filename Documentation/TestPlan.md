# Ilwaco IDE — Test Plan & the document-maintenance rule

Two jobs live in this file:

1. **The document-maintenance rule** — the table below of *which document to update when*. It is
   the rule; the [update-ilwaco-docs](../.claude/skills/update-ilwaco-docs/SKILL.md) skill is how
   to satisfy it without relying on memory, and `Tools/DocCheck.py` is what catches it being
   skipped. Run the check before committing:

   ```bash
   python3 Tools/DocCheck.py
   ```

2. **The forward-looking test plan** — the companion to [Testing.md](Testing.md). Testing.md
   records what **has** been verified; this lists what **should** be, as named scenarios with a
   result against each. Fill in a row in the same commit as any fix it produced.

---

## The rule: which document to update when

The trigger is **any change that could make a document wrong — not just a test.** A removal is the
dangerous case, because it produces no test to force a visit (this is exactly how Astoria's docs
came to describe a deleted Git menu for four days). Treat every row below as a trigger.

| Document | Update when |
| --- | --- |
| `IlwacoIDEManual.md` | user-facing behaviour, a menu, or a workflow changes |
| `Installation.md` | how Ilwaco is **downloaded or installed** changes — a new artefact, a changed filename or version, a new supported distro, or a change to what an upgrade preserves. User-facing, and it ships with the application |
| `IlwacoIDESignificantChanges.md` | a feature is **added or removed** — how Ilwaco differs from VisualFBEditor. On a *removal*, also add a line to `REMOVED_FEATURES` in `Tools/DocCheck.py` |
| `IlwacoVsAstoria.md` | a difference between **Ilwaco and Astoria** appears or disappears — the user-facing comparison. In particular when a *planned* divergence ships, or a gap against Astoria closes. `DocCheck` cannot catch staleness here (the facts live in the other repository), so this row is the only guard |
| `Controls.md` | an MFF control's API, properties, events, or behaviour changes |
| `ControlTesting.md` | a control is tested — record the per-control result |
| `FrameworkFeatures.md` | a non-toolbox framework capability changes |
| `FrameworkGuide.md` | an MFF usage pattern, constructor idiom, or gotcha changes |
| `TechnicalDebt.md` | debt is found, paid down, or a known-suspect area changes |
| `Testing.md` | what is **proven or unproven** changes (typically a test run) |
| `TestPlan.md` | a test scenario is added, run, or made obsolete |
| `UpstreamFixes.md` | a bug in vendored upstream code (VisualFBEditor or Framework) is fixed |
| `AstoriaFindings.md` | a defect or notable flaw is found in **Astoria's own** (Win64) code during the port — the record to feed back to Astoria (distinct from `UpstreamFixes.md`, which is for the shared upstream base) |
| `AstoriaParity.md` | a changelog-walk item is ported, deferred, or reclassified, or its current-status / next-action changes — the port backlog's classification + "Done" record |
| `McpServer.md` | the Agent MCP server changes — a task lands or its status changes, a tool is added, or a design decision is revised |
| `AgentMcpSetup.md` | the *user-facing* side of the MCP server changes — the opt-in toggle, how a client is registered, the advertised tool list, or the security notes |
| `ExamplesAudit.md` | an example is ported, fixed, or found broken — the standing record of which examples build and run on Linux, and why the rest do not |
| `Packaging.md` | anything about shipping Ilwaco changes — what the AppImage bundles, the `Packaging/` scripts, the writable user-data layout, or one of its open decisions is settled |

These are **not** rows in the table above:

- `AstoriaParity.md` **is** now a synchronized doc (row above, per owner 2026-08-04). It stays exempt
  from `DocCheck`'s removed-feature / deleted-file *content* scans — as a port history it names removed
  features and deleted files by design — but it **is** required to appear in the rule table.
- `PROJECT_STATUS.md` (root) — updated after **any** change, per CLAUDE.md; the session handoff. It
  lives at the repo root, outside `Documentation/`, so `DocCheck` does not scan it.
- `AstoriaDetailedChangeLog.md` — the pruned port backlog (the diff to walk); fully excluded from `DocCheck`.
- `CHANGELOG.md` (root) — append a line at each shipped milestone; hand-kept (Ilwaco has no
  PowerShell changelog generator, and `AstoriaDetailedChangeLog.md` already tracks the port work).

### Mark obsolete, do not delete

A test scenario for a removed feature becomes **OBSOLETE** with the date and reason, kept in place.
Deleting it makes coverage that once existed read as coverage that lapsed, and throws away the
reasoning for why it was rewritten.

### Say what is not known

[Testing.md](Testing.md) lists what has been verified **and what has not**, for outside testers. If
something is unproven, write that it is unproven. An honest gap is worth more than a confident
sentence that turns out to be false.

---

## How Ilwaco is verified (Linux/GTK)

Ilwaco has **no headless test harness yet**; verification is *by effect* on the live display, which
shapes what the scenarios below can claim:

- Build: `./build-linux.sh` (`editor` | `lib` | `all`); a clean `fbc` exit is necessary, not
  sufficient — "it compiled" is not "it works".
- Run: `LD_LIBRARY_PATH="$(./build-linux.sh --print-shim)" DISPLAY=:0 ./ilwaco`.
- Observe: `scrot` on `DISPLAY=:0` for screenshots, `xdotool`/`wmctrl` to drive and to raise the
  window (the Claude desktop app steals focus — re-activate the IDE and confirm with
  `xdotool getactivewindow getwindowname` before each click).
- Reset: `git checkout Settings/` after any launch (the IDE writes window state on exit).

---

## Scenarios

Coverage today is thin and mostly manual. This section is the backlog of what to prove; add named
rows (T1, T2, …) as scenarios are defined and run. See [Testing.md](Testing.md) for results.

| # | Scenario | Result |
| --- | --- | --- |
| T1 | Editor builds from source and the window opens on `:0` with no error dialog | **PASS** (standing; see PROJECT_STATUS) |
| T2 | Left/right panels collapse to the vertical rail and re-expand to each tab; pin repaints | **PASS** 2026-08-04 (screenshots) |
| T3 | Designer control library loads (toolbox populates without a runtime error) | **PASS** 2026-08-06 — the Toolbox tab lists the MFF controls (Cursor, Animate, Chart, CheckBox, ComboBoxEdit, …) with their icons, after the library manifest was pruned to the one reachable variant (`LibX64_gtk3`) |
| T4 | A user `.vfp` project builds through the IDE (end-to-end, not `fbc`-direct) | **PASS** 2026-08-04 — console `.vfp` compiled, linked and launched from the IDE (exe deleted beforehand). Needs `CompilationArguments64Linux="-p <shim> -l tinfo"` in *this* dev env only |
| T5 | A project debugs under the Integrated engine: tracing stop, current-line marker, Locals | **PASS** 2026-08-04 |
| T6 | The debuggee receives its arguments and environment (override + inheritance; IDE env untouched) | **PASS** 2026-08-04 (`pgrep -a`, `/proc/<pid>/environ`) |
| T7 | A project under a path containing uppercase letters debugs (path-case regression) | **PASS** 2026-08-04 (`/tmp/ArgTest_MixedCase/`) |
| T8 | Two files differing only in case open as two tabs with their own content | **PASS** 2026-08-04 (`Foo.bas` / `foo.bas`) |
| T9 | Close Project closes the project and leaves the IDE running | **PASS** 2026-08-04 — was a deterministic SIGSEGV; fixed in MFF `TabControl.DeleteTab` |
| T10 | Delete Project removes the project folder from disk | **PASS** 2026-08-04 |
| T11 | Rename Project prompts with the project name and renames the folder | **PASS** 2026-08-04 — dialog has OK + Cancel; folder renamed and project re-opened. Note: the `.vfp` inside keeps its old name (TechnicalDebt) |
| T12 | Debug tabs keep their captions after Use Debugger is toggled off and on | **PASS** 2026-08-04 (regression check for the MFF `_Label` fix) |
| T13 | New Project creates a project on disk from a template, with a working manifest | **PASS** 2026-08-04 — GUI Application → `Project1/` with `Main.frm` + `Project1.vfp`; Console Application → `Project2/` with `Main.bas`. Manifest prefix stripped (`*File=GUI Application/Main.frm` → `*File=Main.frm`); project opens with a populated tree. Was failing: `FileCopy` silently no-ops on `UString` paths (TechnicalDebt) |
| T14 | A project created from a template compiles and prints correctly (BOM regression) | **PASS** 2026-08-04 — **GUI Application**: compiles clean and its window opens with a correctly-rendered caption (no wide-literal BOM effect). **Console Application**: created through New Project in the IDE, then compiled + linked from the IDE ("Layout succeeded"); the built exe prints single-byte ASCII (`Hello, world!` …), exit 0 — the BOM regression check. The old `mff/Console.bi` (Win32-only) blocker is gone: the template was rewritten in plain FreeBASIC and that header deleted. **Note:** the IDE's *run* step then shells out to `gnome-terminal`, absent on this box, so no terminal window opened (build/output verified by launching the exe directly) — a terminal-launcher gap, see TechnicalDebt |
| T15 | Every project type offered by New Project compiles | **PASS** 2026-08-04 — all six offered types compile: GUI, Console, GTK, Dynamic Library, Static Library, Control Library (Console needs `-p <shim> -l tinfo` on the dev box until the AppImage ships `libncurses`) |
| T16 | The agent listener honours "Allow AI agent control (MCP)" without a restart | **PASS** 2026-08-06 — default ON: socket bound, `ping` → `{"pong":true}`, status bar "MCP Agent: On". Untick + OK: socket file gone, `ping` `FileNotFoundError`, status bar "MCP Agent: Off", `AllowAgentControl=false` persisted and reflected when the dialog reopens. Re-tick + OK: socket re-bound live. Restart with the key false: nothing bound at startup |
| T17 | An MCP client reaches the IDE through `ilwaco-mcp`, and is told what to do when it can't | **PASS** 2026-08-06 — with the toggle on, `initialize` → `tools/list` (15 tools) → `tools/call get_status` all succeed over the sidecar's stdio. With it off, the tool call returns `isError` and *"…make sure Tools > Options > Allow AI agent control is ticked"*, and **no second IDE is launched** |
| T18 | An MCP client drives the whole create → build → read-errors → fix → run loop | **PASS** 2026-08-06 — 20 checks from one long-lived `ilwaco-mcp` process with no IDE running beforehand: `create_project` auto-launched exactly one IDE; a sieve with a deliberate typo built to `success=false, error_count=1` with `Variable not declared, composit …` at **line 10**; `get_errors` matched byte for byte; the one-character fix rebuilt clean; the executable printed **`Primes below 1000000 = 78498`**, exit 0; `run` returned in **0.1 s** with the program launched in its own terminal. `run` used to hang forever — see TechnicalDebt "Paid down 2026-08-06". Caveat: the launched terminal renders no output on this box (an xfce4-terminal issue reproducible with `/bin/echo`, TechnicalDebt) |
| T19 | File ▸ Recent Projects… opens the dialog and opens the chosen project | **PASS** 2026-08-06 — the File menu shows a flat "Recent Projects…" (no submenu); the dialog lists File + Path with icons and shows **only** the MRU entry whose `.vfp` still exists (the stale `VisualFBEditor.vfp` is filtered out); OK opened the selected project, confirmed by `get_status` reporting `Framework.vfp` as the open project |
| T20 | Tools ▸ Options: the startup radio group is gone and Code Editor is grouped | **PASS** 2026-08-06 — General no longer shows "When Ilwaco IDE starts"; the Code Editor page shows **Display / Editing / Completion / IntelliSense / History** (Astoria's four with its History catch-all split) with every control inside its frame and no overlapping rows, and the page scrolls to its last row ("Hover time") at the dialog's default 810×640. Getting there took three passes: group boxes need a `Margins.Top` to clear their caption, an explicit `Constraints.Height`, and the label+control panels inside them need one too or GTK compresses the rows into each other |
| T21 | Show Indent Guides draws real indent guides, and the toggle turns them off | **PASS** 2026-08-06 — a four-level nested `.bas` opened in the editor shows a continuous vertical rule at each enclosing indent level (one guide at 8-space indent, two at 12, three at 16, four at 20), distinct from the ShowSpaces dots; unticking **Code Editor ▸ Display ▸ Show Indent Guides** and pressing OK removes them with no restart and writes `ShowIndentGuides=false`. A/B screenshots at 300% |
| T22 | The workspace restores the open project and tabs across a restart | **PASS** 2026-08-06 — a project with two open tabs was closed and the IDE relaunched: `get_status` reported the same project and both tabs, with the previously-selected tab marked `*` in `Settings/Workspace.ini`; paths are stored relative to the executable and the file has no BOM. A workspace naming a since-deleted project opens empty and cleanly, and closing with nothing open removes the stale file |
| T23 | The Sessions UX is gone and the exit save-prompt still works | **PASS** 2026-08-06 — the File menu shows no Open/Save/Close Session and no Recent Sessions; closing with a modified tab still raises the batched save dialog listing `Main.bas*` with Yes/No/Cancel (the renamed `CloseWorkspace`), and answering No exits and writes the workspace; Add From Templates ▸ Recent lists Folders/Projects/Files and selecting Projects lists the recent `.vfp` entries — the category renumbering is right |
| T24 | A `.frm` opens in the form designer and renders the form | **PASS** 2026-08-06 — a GUI Application project's `Main.frm` shows the live form (caption "Main", alignment grid) in the design pane, not Astoria's empty grey panel; a **second** project opened in the same session renders too, so Astoria's "breaking every subsequent project" symptom does not occur here either. This is the evidence behind classifying `cc9e7dd5` **N/A** |
